...
namespace QLSV
{
    ...
    {
        private List<SinhVien> ListDanhSachSinhVien = null;
        public DanhSachSinhVien()
        {
            ListDanhSachSinhVien = new List<SinhVien>();
        }        
        //Hàm tạo ID tăng dần
        private int GernerateID()
        {
            int max = 1;
            if (ListDanhSachSinhVien != null && ListDanhSachSinhVien.Count > 0)
            {
                max = ListDanhSachSinhVien[0].ID;
                foreach(SinhVien sv in ListDanhSachSinhVien)
                {
                    if (max < sv.ID)
                    {
                        max = sv.ID;
        ...
        {
            int Count = 0;
            if (ListDanhSachSinhVien != null)
            {
                Count = ListDanhSachSinhVien.Count;
            }
            return Count;
        }
        ...
        public void NhapSinhVien()
        {
            ...
            ListDanhSachSinhVien.Add(sv);
        }
        public void UpdateSinhVien(int ID)
        {
            // Tìm kiếm sinh viên trong danh sách ListDanhSachSinhVien
        ...
        {
            ListDanhSachSinhVien.Sort(delegate (SinhVien sv1, SinhVien sv2) {
        ...
        public bool DeleteById(int ID)
        {
            ...
            if (sv != null)
            {
                IsDeleted = ListDanhSachSinhVien.Remove(sv);
            }
            return IsDeleted;
        }
         ...
         */
        public List<SinhVien> getListDanhSachSinhVien()
        {
            return ListDanhSachSinhVien;
        }
    }
}
